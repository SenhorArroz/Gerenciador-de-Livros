...
const CATEGORY_OPTIONS = [
  "Curioso", "Cético", "Marcado por cicatrizes", "Estrategista", "Impulsivo", "Leal",
];
...
const decodeInventory = (text: string | null) => {
  ...
  return text.split("\n\n").map((block) => {
    ...
    if (nameMatch) {
      return { name: nameMatch[1], desc: descMatch ? descMatch[1] : "" };
    }
    return { name: block.replace(/^[-*]\s*/, ""), desc: "" };
  });
};
  ...
  return (
    <div className="p-2 relative h-full flex flex-col">
      <div className="flex justify-between items-end mb-1">
        <h1 className="text-[30px] font-semibold text-[#2c2416] leading-tight" style={{ fontFamily: "'Lora', serif" }}>
          Personagens
        </h1>
        <button
      ...
      <p className="text-[15px] text-[#7a6e5f] italic mb-8" style={{ fontFamily: "'Caveat', cursive" }}>
            ...
            <div className="p-8 pl-10">
              <div className="flex justify-between items-start mb-6">
                <h2 className="text-2xl font-semibold text-[#2c2416] flex items-center gap-2" style={{ fontFamily: "'Lora', serif" }}>
                  ...
                  {editingId ? "Editar Ficha do Personagem" : "Nova Ficha de Personagem"}
                </h2>
              ...
              <form className="grid grid-cols-1 md:grid-cols-2 gap-5" onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
                ...
                <div className="md:col-span-2 flex gap-6 mb-2">
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="space-y-1">
                      ...
                      <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full h-11 bg-white border border-[#d1c8b8] rounded-md px-4 focus:outline-none focus:border-[#8e5c3a]" required />
                    </div>
                    <div className="flex gap-3">
                      <div className="space-y-1 flex-1">
                        <label className="text-[11px] font-bold text-[#7a6e5f] uppercase tracking-widest ml-1">Título / Papel</label>
                        <input type="text" value={formData.role} onChange={(e) => setFormData({ ...formData, role: e.target.value })} placeholder="Ex: Protagonista" className="w-full h-11 bg-white border border-[#d1c8b8] rounded-md px-4 focus:outline-none focus:border-[#8e5c3a]" required />
                      </div>
                      <div className="space-y-1 w-24">
                        <label className="text-[11px] font-bold text-[#7a6e5f] uppercase tracking-widest ml-1">Idade</label>
                        <input type="text" value={formData.age} onChange={(e) => setFormData({ ...formData, age: e.target.value })} placeholder="Ex: 28" className="w-full h-11 bg-white border border-[#d1c8b8] rounded-md px-4 focus:outline-none focus:border-[#8e5c3a]" />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="md:col-span-2 space-y-2 mt-2">
                  <label className="text-[11px] font-bold text-[#7a6e5f] uppercase tracking-widest ml-1">Características (Selecione)</label>
                  <div className="flex flex-wrap gap-2 p-3 bg-white border border-[#d1c8b8] rounded-md">
                    {CATEGORY_OPTIONS.map((cat) => (
                      <button key={cat} type="button" onClick={() => toggleCategory(cat)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[13px] font-medium transition-all ${selectedCategories.includes(cat) ? "bg-[#d4d1cc] text-[#2c2416] shadow-sm" : "bg-[#f5f0e8] text-[#7a6e5f] hover:bg-[#eaddce]"}`}>
                ...
                <div className="md:col-span-2 space-y-2">
                  <div className="flex justify-between items-end border-b border-[#e8e4db] pb-1">
                    <label className="text-[11px] font-bold text-[#7a6e5f] uppercase tracking-widest ml-1">Inventário Pessoal</label>
                    <button type="button" onClick={addInventoryField} className="text-[#8e5c3a] hover:bg-[#8e5c3a]/10 px-2 py-1 rounded text-xs font-bold flex items-center gap-1 transition-colors">
                      <Plus size={12} /> Add Item
                    </button>
                  </div>
                  
                  {inventoryFields.length === 0 && (
                    <p className="text-xs text-[#a09586] italic ml-1" style={{ fontFamily: "'Caveat', cursive" }}>Nenhum item na posse do personagem.</p>
                  )}

                  <div className="space-y-3">
                    {inventoryFields.map((field, index) => (
                      <div key={index} className="flex gap-2 items-start bg-black/5 p-3 rounded-md border border-black/5 relative group">
                        <div className="flex-1 space-y-2">
                          <input 
                            type="text" placeholder="Nome do Item (Ex: Amuleto)" value={field.name} 
                            onChange={(e) => updateInventoryField(index, "name", e.target.value)} 
                            className="w-full text-sm bg-transparent border-b border-[#d1c8b8] px-1 py-1 focus:outline-none focus:border-[#8e5c3a]" 
                          />
                          <input 
                            type="text" placeholder="Breve descrição..." value={field.desc} 
                            onChange={(e) => updateInventoryField(index, "desc", e.target.value)} 
                            className="w-full text-xs text-[#7a6e5f] bg-transparent border-none px-1 py-1 focus:outline-none" 
                          />
                        </div>
                        <button type="button" onClick={() => removeInventoryField(index)} className="mt-1 p-1.5 text-red-400 hover:bg-red-100 rounded-md transition-colors">
                          <Minus size={14} />
                        </button>
                      </div>
                    ))}
                ...
                <div className="md:col-span-2 space-y-1">
                  <label className="text-[11px] font-bold text-[#7a6e5f] uppercase tracking-widest ml-1">Notas sobre o Personagem</label>
                  <textarea rows={4} value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} className="w-full bg-white border border-[#d1c8b8] rounded-md px-4 py-3 focus:outline-none focus:border-[#8e5c3a] resize-none" style={{ fontFamily: "'Caveat', cursive", fontSize: "16px" }} />
                </div>

                <div className="md:col-span-2 flex gap-3 mt-4">
                  <button type="button" onClick={handleCloseModal} className="flex-1 h-11 border border-[#d1c8b8] text-[#7a6e5f] rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-black/5">Cancelar</button>
                  <button 
                    type="submit" 
                    disabled={isPending} 
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-4 mb-6 items-center">
        ...
        <div className="text-center col-span-2 flex flex-col justify-end pt-2">
          <h3 className="text-[20px] sm:text-[22px] font-bold text-[#2c2416] leading-tight" style={{ fontFamily: "'Lora', serif" }}>
            {character.name}
          </h3>
        ...
        <div className="col-span-2 flex flex-wrap justify-center items-start gap-1.5">
          {traits.length > 0 && (
            traits.map((trait, index) => (
              <span key={index} className="px-2.5 py-0.5 bg-[#e8e4db]/80 text-[#5a5a5a] rounded-md shadow-[0_1px_2px_rgba(0,0,0,0.05)] text-[12px]" style={{ fontFamily: "'Caveat', cursive" }}>
                {trait}
              </span>
            ))
          )}
            ...
            {globalItems.map((item) => (
              <li key={item.id} className="text-[17px] text-[#2c2416] list-disc marker:text-[#8e5c3a] font-bold" style={{ fontFamily: "'Caveat', cursive", lineHeight: "28px" }}>
                <span className="flex items-center gap-1.5">
                  <Box size={12} className="text-[#8e5c3a] -ml-5 absolute bg-[#fcfaf7]" />
                  {item.name} <span className="text-[14px] text-[#b5a89a] font-normal ml-1">({item.category})</span>
                </span>
              </li>
            ))}
            {inventoryItems.map((item, idx) => (
              <li key={`inv-${idx}`} className="text-[16px] text-[#4a3e2b] list-disc marker:text-[#8e5c3a]" style={{ fontFamily: "'Caveat', cursive", lineHeight: "28px" }}>
                <span className="font-bold">{item.name}</span>
                {item.desc && <span className="text-[14px] text-[#7a6e5f] ml-2">- {item.desc}</span>}
              </li>
            ))}
          </ul>
        </div>
      ...
      {character.description && character.description.trim() !== "" && (
        <div className="mt-auto">
          ...
          <div className="lined-paper-section min-h-[56px] text-[16px] text-[#4a3e2b] pb-2" style={{ fontFamily: "'Caveat', cursive", lineHeight: "28px" }}>
            {character.description.split("\n").map((line, idx) => (
              <div key={idx} className="min-h-[28px] whitespace-pre-wrap">{line}</div>
            ))}
          </div>
        </div>
      )}
